...
def main(args):
    ...
    status = snakemake.snakemake(snakefile,#, configfile=paramsfile,
                                 ...
                                 dryrun=args.dry_run, use_conda=args.use_conda, unlock=args.unlock, printdag=args.dag ), #cores=cores)#, config=config)

    if status: # translate "success" into shell exit code of 0
       return 0
    return 1

    #far funzionare l'opzione "cores"
    
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Python executable for Snakemake workflows', usage='''./run.py -argument
...
''')
    ...
    #parser.add_argument('-q', '--cores', action='store', help='specify the cores to use', type=int, dest='cores')
