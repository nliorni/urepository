...
#print(coupled_columns, single_columns)
...
#df_final[to_change] = df_final[to_change].apply(lambda x: pd.to_numeric(x, errors='coerce'))
# for col in to_change:
#     if col in df_final.columns:
#         df_final[col] = df_final[col].map(lambda x: x.replace("'", ""))
#for col in to_change:
#    df_final[col] = df_final[col].apply(lambda x: pd.to_numeric(x) if (type(x)==int or type(x)==float) and not(np.isnan(x)) else x)
