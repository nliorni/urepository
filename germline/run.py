...
def main(args):
    ...
    status = snakemake.snakemake(snakefile,
                                 ...
                                 force_incomplete=args.reruninc, 
                                 unlock=args.unlock, 
                                 printdag=args.dag,  
                                 cores=args.cores, 
                                 lint=args.lint)
...
-=:___________/
    ...
    parser.add_argument('-w', '--workflow', action='store', help='-[required] choose the workflow. Options: justqc, bam, vcf, vep, cohort', required = True)
    parser.add_argument('-n', '--dry-run', action='store_true', help='-run Snakemake in dry-run mode')
    parser.add_argument('-di', '--directory', action='store', type=str, help='create a working sub-directory with desired name')
    ...
    parser.add_argument('-q', '--cores', action='store', type=int,  help='-[required] specify how many cores to use', required = True)
    parser.add_argument('-f', '--forceall', action='store_true', help='-force the execution of the workflow even if it is done already')
    parser.add_argument('-c', '--configfile', action='store', help='-[required] choose the configuration file', required = True)
    parser.add_argument('-l', '--lint', action='store', type=str, help='print the lint. Default none, can be "plenty" or "json"' )
    parser.add_argument('-wr','--wrapper-prefix', action='store', type=str, help='specify the prefix of your local wrapper repo')
